...
class auto_export_qad:
    ...
    def __init__(self):
        ...
        # self.copydata()
        # self.insert_trans_to_sql(file_path1=file_path1,file_path2=file_path2,table='PurchaseOrder')
    ...
    def insert_trans_to_sql(self, table, file_path1, file_path2):
        try:
            self.logger.debug("Đang đọc dữ liệu từ tệp CSV1...")
            df1 = pd.read_csv(file_path1)
            self.logger.debug("Đang đọc dữ liệu từ tệp CSV2...")
            df2 = pd.read_csv(file_path2)
            # đọc 2 file csv bằng pandas trên rồi mege với nhau thành 1 df
            merged_df = pd.merge(df1, df2, on='PurchaseOrder', how='inner')
            # cộng cột Receipt Qty
            merged_df['OpenQty'] = np.where(
                (merged_df['Status'] != 'X') & (merged_df['Status'] != 'C'),  # Điều kiện
                merged_df['OrderQty'] - merged_df['ReceiptQty'],               # Nếu đúng, trừ hai cột
                0 # Nếu sai, trả về 0
            )
            # tạo cột PO Ord Amt
            merged_df["POOrdAmt"] = merged_df["POOrdCost"] * merged_df["OrderQty"]
            merged_df['OrderDate'] = pd.to_datetime(merged_df['OrderDate'], format='%d/%m/%y', errors='coerce')
            merged_df['DueDate'] = pd.to_datetime(merged_df['OrderDate'], format='%d/%m/%y', errors='coerce')

            # kiểm tra có bảng hay chưa nếu có xóa dữ liệu
            ...
            merged_df.to_sql(table, con=conect, if_exists='append', index=False)
    ...
    def open_progess_editor(self):
        try:
            ...
            self.logger.error(f"Lỗi: {e}")
            email_body = f"<p>Lỗi trong quá trình xuất: {str(e)}</p>"
            self.send_email("Export auto PO QAD Error:", email_body, "tvc_adm_it@terumo.co.jp")
    ...
    def export_po(self):
        try:
            ...
            # nhập code
            code = f'''
            OUTPUT TO "/home/mfg/pod_det_export.csv".
            PUT UNFORMATTED "PurchaseOrder,Poln,ItemNumber,DueDate,OrderQty,Status,POOrdCost,ReceiptQty" SKIP.
            FOR EACH pod_det NO-LOCK:
            PUT UNFORMATTED
            STRING(pod_det.pod_nbr) + "," + 
            STRING(pod_det.pod_line) + "," + 
            STRING(pod_det.pod_part) + "," + 
            STRING(pod_det.pod_due_date, "99/99/99") + "," + 
            STRING(pod_det.pod_qty_ord) + "," + 
            STRING(pod_det.pod_status) + "," + 
            STRING(pod_det.pod_pur_cost) + "," + 
            STRING(pod_det.pod_qty_rcvd) SKIP.
            END.
            OUTPUT CLOSE.

            OUTPUT TO "/home/mfg/po_mstr_export.csv".
            PUT UNFORMATTED "PurchaseOrder,Supplier,OrderDate,Curr" SKIP.
            FOR EACH po_mstr NO-LOCK:
            PUT UNFORMATTED 
            STRING(po_mstr.po_nbr) + "," + 
            STRING(po_mstr.po_vend) + "," + 
            STRING(po_mstr.po_ord_date, "99/99/99") + "," + 
            STRING(po_mstr.po_curr) SKIP.
            END.
            OUTPUT CLOSE.
            '''
            file_path1=r"Z:\pod_det_export.csv"
